...
const mobileIconClickDuration = 0.6;
...
export function PremiumBanner() {
	...
	const { hoverDuration, hoverLift, hoverScale, isMobile, prefersReducedMotion } = useMotionProfile();
			...
			<Container className="px-4 sm:px-6 lg:px-8">
									...
									>
										...
										<motion.button
											animate={pendingMobileHref === href ? {
												rotateY: prefersReducedMotion ? 0 : [0, 960, 1920, 2880],
											} : {
													rotateY: 0,
											}}
				...
				>
						...
						<motion.div key={label} variants={createScaleInVariants(index, false)} className="min-w-[148px] shrink-0 sm:min-w-0">
							{href ? (
								<motion.div
										whileHover={{ y: hoverLift, scale: hoverScale, boxShadow: "0 0 25px rgba(168,85,247,0.3)" }}
									whileTap={{ scale: 0.96 }}
									transition={{ duration: hoverDuration, ease: snappyEase }}
								>
									<Link
										href={href}
										className={`group card-premium relative flex w-full items-center gap-3 overflow-hidden rounded-[1.4rem] border bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))] px-3 py-3 text-left backdrop-blur-sm transition sm:gap-4 sm:rounded-[1.6rem] sm:px-4 sm:py-4 ${border} ${glow}`}
									>
										<div className={`absolute inset-0 bg-linear-to-r ${accent} opacity-90 transition duration-300`} />
										...
										<div className={`relative flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-white/12 ${iconSurface} transition duration-300 group-active:scale-95 sm:h-16 sm:w-16 sm:group-hover:scale-105 sm:group-hover:shadow-[0_0_32px_rgba(255,255,255,0.12)]`}>
								...
								<motion.button
									...
									transition={{ duration: hoverDuration, ease: snappyEase }}
									type="button"
									className={`group card-premium relative flex w-full items-center gap-3 overflow-hidden rounded-[1.4rem] border bg-white/[0.03] px-3 py-3 text-left backdrop-blur-sm transition sm:gap-4 sm:rounded-[1.6rem] sm:px-4 sm:py-4 ${border} ${glow}`}
								>
									<div className={`absolute inset-0 bg-linear-to-r ${accent} opacity-90 transition duration-300`} />
									<div className="absolute inset-x-0 top-0 h-px bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.28),transparent)] opacity-70" />
									<div className={`relative flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-white/12 ${iconSurface} transition duration-300 group-active:scale-95 sm:h-16 sm:w-16 sm:group-hover:scale-105 sm:group-hover:shadow-[0_0_32px_rgba(255,255,255,0.12)]`}>
